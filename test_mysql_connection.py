#!/usr/bin/env python3
"""
Test MySQL database connection from Windows runner to Linux container database.
"""
...
def test_connection(max_retries=5, retry_delay=3):
    """
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(f"✓ Inserted test record with ID={last_id}")
            ...
            cursor.execute("SELECT id, name, created_at FROM test_table WHERE id = %s", (last_id,))
            row = cursor.fetchone()
            print(f"✓ Retrieved record: ID={row[0]}, Name={row[1]}, Created={row[2]}")
